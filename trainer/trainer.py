    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(self.data_loader):
            ...
            kl_raw, nll_raw, kl_fr, nll_fr, kl_m, nll_m, loss = \
                self.criterion(x, x_recon, sigma2_noise, mu_q_seq, logvar_q_seq, mu_p_seq, logvar_p_seq, kl_annealing_factor, x_mask)
            loss.backward()
                ...
                for met in self.metric_ftns:
                    if met.__name__ == 'bound_eval':
                        self.train_metrics.update(met.__name__,
                                                  met([x_recon, sigma2_noise, mu_q_seq, logvar_q_seq],
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(self.valid_data_loader):
                ...
                x_recon, sigma2_noise, z_q_seq, z_p_seq, mu_q_seq, logvar_q_seq, mu_p_seq, logvar_p_seq = \
                    self.model(x, x_reversed, x_seq_lengths)
                kl_raw, nll_raw, kl_fr, nll_fr, kl_m, nll_m, loss = \
                    self.criterion(x, x_recon, sigma2_noise, mu_q_seq, logvar_q_seq, mu_p_seq, logvar_p_seq, 1, x_mask)
